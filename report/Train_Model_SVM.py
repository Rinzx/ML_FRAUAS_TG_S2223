...
for model in models:
    ...
    # Generate ROC curves and calculate AUC scores (svm.SVC)
    model_fpr, model_tpr, model_thresholds = roc_curve(y_test, model.decision_function(X_test))
    model_auc = auc(model_fpr, model_tpr)
        
    # Plot ROC curves
    plt.figure()
    plt.plot(model_fpr, model_tpr, label='SVM (AUC={:.2f})'.format(model_auc))
